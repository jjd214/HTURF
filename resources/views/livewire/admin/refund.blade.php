        ...
        <table class="table table-hover stripe" style="width: 100%;" wire:poll.keep-alive>
            ...
            <tbody>
                @forelse ($rows as $item)
                    <tr style="cursor: pointer;" wire:click="selectRefund({{ $item->id }})" data-toggle="modal"
                        data-target="#refundModal">
                        <td><small><b>{{ $item->transaction_code }}</b></small></td>
                        <td><span
                                class="badge {{ $item->status == 'Refunded' ? 'badge-danger' : 'badge-success' }}">{{ $item->status }}</span>
                        </td>
                        ...
                        <div class="card p-3 border-0">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <p class="ml-4">
                                        <span
                                            class="badge {{ $selectedRefund->status == 'Refunded' ? 'badge-danger' : 'badge-success' }}">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    @if ($selectedInventory)
                                        <p class="ml-4 text-muted"><small><b>Item:</b>
                                                {{ $selectedInventory->name }}</small></p>
                                        <p class="ml-4 text-muted"><small><b>Sku:</b>
                                                {{ $selectedInventory->sku }}</small></p>
                                        <p class="ml-4 text-muted"><small><b>Brand:</b>
                                                {{ $selectedInventory->brand }}</small></p>
                                        <p class="ml-4 text-muted"><small><b>Size:</b>
                                                {{ $selectedInventory->size }}</small></p>
                                        <p class="ml-4 text-muted"><small><b>Color:</b>
                                                {{ $selectedInventory->color }}</small></p>
                                    @else
                                ...
                                <div class="col-md-6 mb-3">
                                    <strong><i class="fa fa-question-circle mr-2 text-primary"></i>Refund
                                        Reason:</strong>
